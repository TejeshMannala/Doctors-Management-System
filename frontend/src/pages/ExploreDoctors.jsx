      ...
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div className="mt-8 rounded-[28px] bg-white p-8 shadow-sm">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div className="md:col-span-2 xl:col-span-1">
              <label className="form-label">{t('Search Doctor')}</label>
              <input
                type="text"
                value={nameFilter}
                onChange={(e) => setNameFilter(e.target.value)}
                className="form-input"
                placeholder={t('Search by doctor name or email')}
              />
            </div>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">
            ...
            {(nameFilter || specializationFilter || experienceFilter || feeFilter) && (
              <button
                type="button"
        ...
        {showGuardModal && location.state?.bookingGuardMessage && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4">
            <div className="w-full max-w-lg rounded-[30px] border border-amber-200 bg-white p-8 shadow-2xl">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-600">{t('Booking Notice')}</p>
              <h2 className="mt-3 text-2xl font-bold text-slate-950">{t('Please select a doctor first')}</h2>
