...
const AppointmentConfirmation = () => {
    ...
    const fetchActiveAppointment = async () => {
      ...
      try {
        ...
        if (activeAppointment) {
          setBookingBlockMessage(
            `${t('You already have an active appointment')} (${activeAppointment.status}) ${t('on')} ${formatDate(activeAppointment.date, i18n.resolvedLanguage)}. ${t('Please complete or cancel it before booking another one.')}`
          );
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      setErrors({
        submit: error.response?.data?.message || t('Failed to book appointment. Please try again.'),
      });
    } finally {
  ...
  if (bookingSuccess && bookedAppointment) {
    return (
      <div className="min-h-screen bg-sky-600 px-4 py-10">
        <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-xl items-center justify-center">
          <div className="w-full overflow-hidden rounded-[28px] bg-white text-center shadow-[0_24px_80px_-25px_rgba(15,23,42,0.55)]">
            ...
            <div className="px-8 pb-8">
              <div className="rounded-2xl bg-slate-50 p-4 text-left text-sm text-slate-700">
                <p><strong>{t('Appointment ID')}:</strong> {bookedAppointment._id}</p>
                <p><strong>{t('Doctor Name')}:</strong> {doctor.userId?.fullName}</p>
                <p><strong>{t('Date & Time')}:</strong> {new Date(bookedAppointment.date).toLocaleDateString()} at {bookedAppointment.timeSlot}</p>
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(191,212,255,0.45),_transparent_35%),linear-gradient(180deg,#f8fbff_0%,#f8fafc_50%,#eef2ff_100%)] py-8">
      <div className="mx-auto max-w-6xl px-4">
        ...
        {showBookingBlockModal && bookingBlockMessage && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-sky-600/90 px-4">
            <div className="w-full max-w-xl rounded-[28px] bg-white p-8 text-center shadow-2xl">
              <div className="mx-auto mb-6 flex h-28 w-28 items-center justify-center rounded-full bg-sky-50">
                <div className="flex h-20 w-20 items-center justify-center rounded-full bg-sky-500 text-5xl text-white shadow-lg">
                  !
                </div>
              </div>
              <h2 className="text-4xl font-bold tracking-tight text-slate-900">{t('Ooops!')}</h2>
              ...
              <button
                type="button"
                className="mt-8 inline-flex min-w-[180px] items-center justify-center rounded-xl bg-primary-600 px-6 py-3 text-base font-semibold text-white shadow-md transition hover:bg-primary-700"
              ...
              <div className="mt-6 space-y-5">
                ...
                <button
                  ...
                  disabled={
                    isSubmitting ||
                    checkingBooking ||
                    Boolean(bookingBlockMessage) ||
                    !appointmentData.date ||
                    !appointmentData.timeSlot
                  }
                >
                  {checkingBooking
                    ? t('Checking booking status...')
                    : isSubmitting
                      ? t('Booking...')
                      : t('Confirm Booking')}
                </button>
